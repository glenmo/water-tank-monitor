...
IPAddress staticIP(192, 168, 55, 172);     // Arduino IP
IPAddress gateway(192, 168, 55, 1);        // Router IP  
IPAddress subnet(255, 255, 255, 0);        // Subnet mask
IPAddress dns(192, 168, 55, 1);            // DNS server
...
void connectWiFi() {
  ...
  // CRITICAL: Configure static IP BEFORE WiFi.begin()
  // The Arduino WiFiS3 library requires this order
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Verify IP is valid
    if (ip[0] == 0) {
      Serial.println(F("⚠ ERROR: Still no valid IP address!"));
      Serial.println(F("Check: Router allows static IPs?"));
    } else {
      Serial.println(F("✓ WiFi ready!"));
    }
  } else {
    Serial.println(F("WiFi connection failed!"));
    Serial.println(F("Check: SSID and password correct?"));
  }
}
...
void onEvent(ev_t ev) {
  ...
  switch(ev) {
    ...
    case EV_JOINED:
      ...
      loraJoined = true;
      loraSending = false;
      lastLoRaUploadTime = millis();                    // LoRa fires in 60s
      lastWiFiUploadTime = millis() - 30000;            // WiFi fires in 30s (offset!)
      LMIC_setLinkCheckMode(0);
      
      // NOW safe to start WiFi - LoRa join is complete, SPI is free
      Serial.println();
      Serial.println(F("--- Connecting WiFi (post-join) ---"));
      connectWiFi();
      break;
    case EV_JOIN_FAILED:
...
void setup() {
  ...
  // Arduino R4 has different SPI timing than classic AVR.
  // The SX1276 sometimes doesn't fully reset, causing LMIC's
  // state machine to get stuck (ASSERT at radio_sx127x.c:1065).
  // Fix: manually pulse RST pin AFTER LMIC_reset() and wait
  // long enough for the radio to fully settle before joining.
  Serial.println(F("Resetting radio (post-LMIC)..."));
  pinMode(9, OUTPUT);
  digitalWrite(9, LOW);
  delay(200);            // Hold reset LOW longer than default
  digitalWrite(9, HIGH);
  delay(500);            // Wait for radio to fully power up and settle
  
  // Re-init LMIC after manual reset so its state matches the radio
  LMIC_reset();
  delay(100);
  
  Serial.println(F("LoRaWAN stack initialized"));
  ...
  LMIC_setClockError(MAX_CLOCK_ERROR * 100 / 100);  // 100% tolerance for R4 timing
  
  Serial.println(F("Starting OTAA join..."));
  LMIC_startJoining();
  
  // WiFi is intentionally NOT started here.
  // It will start AFTER LoRa has successfully joined
  // to avoid SPI bus conflicts during the join window.
  Serial.println(F("WiFi will connect after LoRa join..."));
